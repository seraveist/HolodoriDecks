...
- 리더/멤버 동일 홀로멤 분리 옵션
- `유닛 스코어` / `악곡 기대 스코어` / `악곡 최대 스코어` 목표별 TOP 5 자동 추천
- 악곡 목표에서 악곡·난이도·AUTO 플레이 / 수동 ALL PERFECT 조건 선택
...
3. 육성 반영과 `리더를 편성에 제외` 여부를 설정합니다. 제외를 켜면 리더와 같은 홀로멤의 카드를 멤버로 사용할 수 없습니다.
4. `계산 설정`에서 세 목표 중 하나를 선택합니다. 악곡 목표에서는 악곡·난이도·플레이 기준도 선택합니다.
...
- **유닛 스코어:** 악곡과 무관한 범용 유닛 스코어로 조합을 평가합니다.
- **악곡 기대 스코어:** 선택한 악곡의 스킬 발동 확률을 반영한 기대 점수로 평가합니다.
- **악곡 최대 스코어:** 선택한 악곡에서 모든 유효 액티브가 성공하는 조건의 점수로 평가합니다.

유닛 스코어에서는 악곡·난이도·플레이 기준을 숨기고 계산에도 사용하지 않습니다. 악곡 목표로 돌아오면 이전 악곡 설정을 복원합니다. 악곡 목표에서 곡을 선택하지 않은 경우 계산 버튼이 비활성화됩니다. 기존 저장 데이터의 목표·악곡 조합은 새 목표로 자동 이전됩니다.

안정적인 덱과 이상적인 고점이 높은 덱은 서로 다른 TOP 5를 반환할 수 있습니다.

SP 스킬의 효과 자체는 범용 유닛스코어에도 포함하지만, 특정 악곡의 발동 위치와 배치 이득은 곡별 계산에서만 반영합니다.

유닛 스코어 결과는 **같은 리더 + 같은 멤버 5명 조합당 대표 순서 하나**를 표시합니다. 조합은 범용 유닛 점수로 순위를 정하고, 각 조합의 대표 순서는 공통 채보의 잠재 점수로 120가지 순서를 비교해 선택합니다. 멤버 프리셋은 해당 카드의 포함을 보장하며 배치 위치는 고정하지 않습니다.

공통 채보는 110초·800개의 균등한 탭 노트·구간을 6등분한 경계의 SP 5개 지점(약 18.33·36.67·55·73.33·91.67초)을 가정하고 AUTO 기준으로 고정합니다. 저장된 악곡·난이도·플레이 기준은 이 공통 기준을 바꾸지 않습니다. 공통 채보 잠재 스코어는 대표 배치를 고르는 내부 기준으로만 사용합니다. 인게임 비교용 유닛스코어와 잠재 유닛스코어를 덮어쓰지 않으며, 악곡 목표에서는 해당 곡의 시간축과 선택한 계산 목표로 다시 배치합니다.
...
악곡 선택 결과도 같은 리더·멤버 5명 조합당 대표 순서 하나만 표시합니다. `악곡 기대 스코어`는 기대 점수가 최대인 순서를, `악곡 최대 스코어`는 모든 유효 액티브 성공 조건의 점수가 최대인 순서를 120가지 순서 중에서 선택합니다. 한 결과에 표시하는 기대·최대 점수는 모두 그 대표 순서의 값입니다. Master/Estimated에서는 해당 근사 모델의 최댓값을 사용합니다.
...
```text
unit-score-v1.0-verified-display + song-score-v0.5-independent-song-base
```
...
- 악곡 재생시간·스코어 계수
- Master 풀콤보 노트 수
- 수동 ALL PERFECT의 PERFECT 노트 계수와 콤보 보너스
- Exact 채보의 실제 노트 시각·노트 타입·SP 슬롯 시각

악곡 점수는 종합력에 `K`를 적용한 스킬 제외 기준값에서 시작하고, 선택 채보의 노트·스킬 배율을 적용합니다. 환산 기준은 110초·800노트·계수 5의 AUTO로 고정하여 수동 ALL PERFECT의 노트 계수와 콤보 보너스를 보존합니다. 범용 표시용 스킬 보너스를 악곡 점수에 재사용하지 않으며, 최대 스코어는 발동 가능한 스킬의 모든 판정이 성공한 경우를 계산합니다. 채보가 없는 경우에도 액티브 주기·지속시간·중첩·곡 종료 시점은 시간 구간별로 계산하고, 노트 분포와 SP 효과는 평균값으로 근사합니다.
...
Local Exact도 악곡·난이도·해시·노트 수와 순서·SP 5개 슬롯을 검증합니다. 불완전하거나 다른 채보의 파일이면 Runtime Exact를 시도한 뒤 Master로 전환합니다.
...
- 수동 모드는 모든 노트가 PERFECT인 **수동 ALL PERFECT** 조건입니다.
