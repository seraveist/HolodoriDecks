...
- 언어 드롭다운 위에 같은 너비의 일반 `LANGUAGE` 헤더 텍스트 추가
- `/ko/`, `/en/`, `/ja/`의 번역된 정적 HTML과 언어별 제목·설명, canonical, hreflang, 사이트맵 및 robots.txt 생성
- 언어 URL을 우선 적용하고 언어 전환 시 같은 도메인의 보유 카드 저장값 유지
- 사이트 하단에 언어 링크, 비공식 도구 표시와 브라우저 저장 안내 추가

### Security

- 캐릭터명이 멤버 버튼의 HTML 속성으로 삽입되지 않도록 이스케이프 처리
- 인라인 테마 스크립트를 외부 파일로 옮기고 스크립트 출처를 제한하는 CSP 및 referrer 정책 적용
- Pages 배포를 main으로 제한하고 외부 GitHub Actions를 확인된 커밋 SHA로 고정
- 자동 동기화에서 검증을 통과한 정확한 커밋에 `validate` 상태를 기록해 main 필수 검증과 연동
- 다국어 HTML, 배포 결과와 HTML 속성 주입 방지를 검사하는 회귀 검증 추가

### Fixed

- 과거 유닛 스코어 검증의 입력 데이터와 의존 모듈을 당시 스냅샷으로 고정해 신규 Master 데이터 동기화 시 발생하던 검증 실패를 수정
- 브라우저 검증에서 Chrome 시작 실패만 새 프로필로 재시도하고, 디버깅 포트 파일 확인과 오류 기록을 보완
- Master 동기화 후보를 별도 브랜치에서 전체 검증한 후에만 PR 브랜치를 갱신하도록 변경하고, 실제 병합 완료 여부로 배포 조건을 판정

## [1.3.0] - 2026-09-14

### Changed

- 계산 설정을 유닛 스코어·악곡 기대 스코어·악곡 최대 스코어의 세 가지 목표로 통합하고, 악곡 목표에서만 악곡·난이도·플레이 조건을 선택하도록 정리
- 플레이 조건을 AUTO 플레이·수동 ALL PERFECT로 표시하고 리더 옵션을 '리더를 편성에 제외'로 변경
- 목표 전환 시 악곡 선택을 보존하고 기존 저장 설정을 이전하며, 작은 화면에서도 육성 기준과 리더 제외 옵션을 한 행에 배치
- 화면과 계산 모듈의 캐시 버전을 릴리스 버전으로 통일

### Fixed

- 악곡 점수를 스킬 제외 종합력에서 계산하고 AUTO 기준 환산을 고정해, 수동 ALL PERFECT의 노트·콤보 보너스가 상쇄되는 문제를 수정
- 최대 스코어에 발동 확률이 간접 반영되거나 발동 불가능한 스킬이 성공 처리되는 문제를 수정
- 근사 계산에서도 액티브의 실제 주기·중첩·종료 구간을 적분해 기대·최대 스코어 역전과 탐색·최종 표시 불일치를 수정
- 마지막 노트에서 아직 지속 중인 액티브·SP 효과가 조기 종료되는 문제를 수정
- Local Exact의 노트·SP 슬롯과 채보 식별 정보를 검증하고 불완전한 데이터는 Runtime Exact·Master 순으로 대체

### Validation

- 199곡·796개 난이도의 Master·Estimated 경로와 고정 snapshot의 Runtime Exact 699개에서 기대·최대 점수, AUTO·수동, SP 반영 및 단일 목표·최종 결과 일치 검사 통과
- 실제 채보와 Master 추천 사례의 TOP 5를 전체 조합·120순서 전수 계산과 비교하고 기존 보유 카드 5개 조건의 추천 회귀 검사 통과
- 기존 유닛스코어 실측 회귀값을 유지하며, 브라우저의 세 목표·설정 이전·리더 제외·다국어 모바일·Exact 대체 경로 검사 통과

## [1.2.3] - 2026-09-13

### Changed

- 카드 상세 팝업의 스킬을 한 열 목록으로 정리하고, 종류별 색상과 발동률 배지로 정보를 구분
- 제목·설명·레벨·능력치의 글꼴 크기와 굵기를 메인 화면의 공통 규격에 맞춤
- 모바일의 카드 이미지·능력치 배치를 압축하고, 중복되는 카드 표시와 동일한 MAX 수치를 생략

## [1.2.2] - 2026-09-13

### Fixed

- 초기 순위가 낮은 리더에도 멤버 교체 탐색을 적용하고, 다른 리더에서 발견한 우수한 멤버 조합을 교차 비교해 상위 편성 누락을 보완
- 범용 추천에서 순서별 패시브 대상 차이로 점수가 오를 수 있는 조합을 최종 120개 순서 비교까지 유지
- 보유 카드 선택·가져오기 순서가 같지 않아도 동일한 추천 결과가 나오도록 탐색 입력 순서를 통일
- 전체 요청의 전수 탐색량을 제한해 중간 규모 보유 조건에서 계산량이 급증하는 문제를 수정
- Worker 시간 초과·오류 시 메인 화면에서 계산을 다시 시작하던 동작을 제거하고, 작업 취소 및 실패 처리를 보완

### Validation

- 고정된 5개 보유 조건의 기대·잠재 TOP5 회귀 검사 추가. 26장·14장 조건은 독립 전수 검사 결과와 일치하며, 나머지 조건은 기존 독립 탐색에서 확인한 TOP5 점수 기준을 충족
- 150개 편성의 순서별 점수 범위를 전체 120개 순서와 비교하는 검사 및 Worker 실패·취소 검사 추가
- 실제 브라우저의 5개 보유 조건과 기존 악곡·프리셋·스코어 검사를 통과하고 배포 전 자동 검사에 반영

## [1.2.1] - 2026-09-13

### Removed

- 추천 결과의 추정값·계정 효과 미반영 안내를 한국어·영어·일본어에서 삭제
- 악곡 미선택 결과의 공통 채보 잠재 스코어·추천 배치 설명·SP 배치 박스를 삭제. 조합별 최적 순서 선택 계산은 유지
