...
def build_targets() -> list[AssetTarget]:
    ...
    for card in cards:
        if card.get("rarity") not in SUPPORTED_CARD_RARITIES:
            continue
...
def _load_overrides(source_root: Path, path: Path | None) -> dict[str, Path]:
    if path is None or not path.exists():
        return {}

    payload = _load_json(path)
    return {
        str(card_id): source_root / str(relative_path)
        for card_id, relative_path in payload.get("cards", {}).items()
    }


def _convert_to_webp(source: Path, destination: Path, quality: int) -> None:
    try:
        from PIL import Image, ImageOps
    except ImportError as exc:  # pragma: no cover - optional dependency
        raise RuntimeError("Card image import requires Pillow. Install with: pip install -e '.[assets]'") from exc
...
def import_assets(
    ...
    *,
    ...
    for target in targets:
        ...
        override = overrides.get(target.item_id)
        ...
        if not candidates:
            missing.append({"id": target.item_id, "asset_id": target.upstream_asset_id})
            continue
        ...
        source = candidates[0]
        if not dry_run:
            _convert_to_webp(source, target.destination, quality=quality)
    ...
    return {
        ...
        "supported_rarities": sorted(SUPPORTED_CARD_RARITIES),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Import locally extracted rarity-4/5 card portraits into static GitHub Pages assets"
    )
    parser.add_argument("source", type=Path, help="root folder containing extracted card image files")
    parser.add_argument(
        "--map",
        dest="overrides_path",
        type=Path,
        default=ASSET_DIR / "source-map.json",
        help="optional explicit card source mapping JSON",
    )
